...
if(global.gameIsPaused == false) {
	...
	//НУ ЭТО КОСТЫЛЬ Я НЕ СПОРЮ
	if(instance_exists(obj_player)){
	x_for_pause = instance_find(obj_player, 0).x
	y_for_pause = instance_find(obj_player, 0).y
	}
	//ЧТОБЫ СДЕЛАТЬ ВОТ ЭТО
	if(instance_exists(obj_player_otjimania)){
	x_for_pause = instance_find(obj_player_otjimania, 0).x
	y_for_pause = instance_find(obj_player_otjimania, 0).y
	}
...
if(global.gameIsPaused){
	instance_create_layer(x_for_pause, y_for_pause - 50, "UI", obj_resumebutton);
	instance_create_layer(x_for_pause, y_for_pause + 50, "UI", obj_mainmenubutton);
