...
switch(state){
	...
	case e_state.patrol:
	{
		if(obj_enemy_weak.x >= en_x - 75 && obj_enemy_weak.x <= en_x + 75 ){
			dir = -1;
			hsp = 2.5 * dir;
			vsp = min(7, vsp + 0.5);
			if(obj_enemy_weak.x >= en_x - 75 && obj_enemy_weak.x <= en_x - 75){
